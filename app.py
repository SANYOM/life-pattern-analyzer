...
.metric-value { font-size: 2.4rem; font-weight: 800; color: white; line-height: 1; }
.badge {
...
[data-testid="stTabs"] button { color: rgba(255,255,255,0.5) !important; font-weight: 600; }
...
MODEL_FILES = [
    ...
    "feature_cols.pkl", "peer_data.pkl",
]

@st.cache_resource(show_spinner=False)
def load_all_models():
    if not all(os.path.exists(f) for f in MODEL_FILES):
        with st.spinner("🧠 Training your Digital Twin AI… (first run only, ~20s)"):
            try:
                import model as m
                m.train_all_models()
...
def predict_all(mdls, inputs):
    ...
    pm = {1: ("Low", "green"), 2: ("Medium", "amber"), 3: ("High", "red")}
    label, color = pm.get(pressure_raw, ("Medium", "amber"))
    return dict(
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART HELPERS
# KEY FIX: _base_layout uses layout.update(kwargs) so passed kwargs override
# defaults — no more "dict() got multiple values for keyword argument margin"
# ─────────────────────────────────────────────────────────────────────────────
...
def projection_chart(df_proj):
    ...
    fig.add_trace(go.Scatter(
        x=df_proj["day"], y=df_proj["burnout"],
        name="Burnout Risk", mode="lines+markers",
        line=dict(color="#f87171", width=2.5),
        marker=dict(size=8), fill="tozeroy",
        fillcolor="rgba(248,113,113,0.08)",
    ))
    fig.add_trace(go.Scatter(
        x=df_proj["day"], y=df_proj["productivity"],
        name="Productivity", mode="lines+markers",
        line=dict(color="#60a5fa", width=2.5), marker=dict(size=8),
    ))
    fig.add_trace(go.Scatter(
        x=df_proj["day"], y=df_proj["wellbeing"],
        name="Wellbeing", mode="lines+markers",
        line=dict(color="#34d399", width=2.5), marker=dict(size=8),
    ))
...
def simulate_7_days(mdls, base):
    ...
    for day in DAYS:
        ...
        fatigue += 0.45 if p["burnout"] > 65 else (0.18 if p["burnout"] > 40 else -0.25)
        fatigue = max(0.0, fatigue)
        rows.append(dict(day=day,
                         burnout=round(p["burnout"], 1),
...
with st.sidebar:
    ...
    mll = ("Sustainable 🟢" if ml < 33 else
           "Moderate 🟡"    if ml < 55 else
           "High 🔴"        if ml < 75 else "Critical 🚨")

    st.markdown('<hr class="divider">', unsafe_allow_html=True)
    st.markdown("### ⚡ Mental Load Index")
    st.markdown(f"""
    <div style='font-size:1.6rem;font-weight:800;color:{mlc}'>{ml:.0f}
      <span style='font-size:1rem;color:rgba(255,255,255,0.4)'>/100</span>
    </div>
    <div style='background:rgba(255,255,255,0.08);border-radius:99px;height:10px;margin-top:6px'>
      <div style='width:{ml}%;background:{mlc};height:10px;border-radius:99px'></div>
    </div>
    <div style='color:rgba(255,255,255,0.5);font-size:0.82rem;margin-top:6px'>{mll}</div>
    """, unsafe_allow_html=True)

    if ml >= 75:
        st.error("🚨 Critical overload! Check the Insights tab.")
...
st.markdown(
    '<div class="hero-sub">Your personal Digital Twin — live predictions · 7-day simulation · smart insights</div>',
    unsafe_allow_html=True,
)
...
with tab1:
    ...
    with c1:
        ...
        <div class='glass-card'>
          ...
          <div style='margin-top:6px'><span class='badge {bc}'>
            {'Low' if preds['burnout']<33 else 'Moderate' if preds['burnout']<66 else 'High'}
          </span></div>
    ...
    with c2:
        ...
        <div class='glass-card'>
          ...
          <div style='margin-top:6px'><span class='badge {pc}'>
            {'Low' if preds['productivity']<33 else 'Moderate' if preds['productivity']<66 else 'High'}
          </span></div>
        </div>""", unsafe_allow_html=True)
    ...
    with c3:
        ...
        <div class='glass-card'>
          ...
          <div style='margin-top:6px'><span class='badge {wc}'>
            {'Low' if preds['wellbeing']<33 else 'Moderate' if preds['wellbeing']<66 else 'High'}
          </span></div>
        </div>""", unsafe_allow_html=True)
    ...
    with c4:
        ...
        <div class='glass-card'>
          ...
          <div style='margin-top:6px'><span class='badge {ac}'>AI Classified</span></div>
        </div>""", unsafe_allow_html=True)
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("<div class='glass-card'><div class='metric-label'>🧬 Your Habit DNA Fingerprint — vs Ideal Zone</div>",
                unsafe_allow_html=True)
    st.plotly_chart(
...
with tab2:
    ...
    st.markdown("<p style='color:rgba(255,255,255,0.45)'>Pick any habit and drag the slider to instantly see how that single change reshapes all your metrics.</p>",
                unsafe_allow_html=True)

    habit = st.selectbox("Which habit do you want to change?", [
        "Sleep Hours 🛏️", "Screen Time 📱", "Study Hours 📚",
        "Caffeine Intake ☕", "Stress Level 🧠", "Physical Activity 🏃",
    ])

    sim_inputs = list(INPUTS)

    if habit == "Sleep Hours 🛏️":
        nv = st.slider("New Sleep Hours", 0, 12, sleep)
        sim_inputs[2] = nv
        change_desc = f"Sleep {sleep}h → {nv}h"
    elif habit == "Screen Time 📱":
        nv = st.slider("New Screen Time (hrs)", 0, 12, screen)
        sim_inputs[3] = nv
        change_desc = f"Screen {screen}h → {nv}h"
    elif habit == "Study Hours 📚":
        nv = st.slider("New Study Hours", 0, 12, study)
        sim_inputs[5] = nv
        change_desc = f"Study {study}h → {nv}h"
    elif habit == "Caffeine Intake ☕":
        nv = st.slider("New Caffeine (cups)", 0, 5, caffeine)
        sim_inputs[7] = nv
        change_desc = f"Caffeine {caffeine} → {nv} cups"
    elif habit == "Stress Level 🧠":
        ns = st.selectbox("New Stress Level", ["Low", "Medium", "High"])
        sim_inputs[4] = {"low": 1, "medium": 2, "high": 3}[ns.lower()]
        change_desc = f"Stress {stress_raw} → {ns}"
    else:
        np2 = st.selectbox("New Physical Activity", ["Yes", "No"])
        sim_inputs[6] = 1 if np2 == "Yes" else 0
        change_desc = f"Activity {phys_raw} → {np2}"
    ...
    if net > 5:
        st.markdown(f"<div class='insight-good'>✅ <b>Positive impact.</b> This change improves your overall life score by <b>{net:.1f} points</b>. Highly recommended!</div>",
                    unsafe_allow_html=True)
    elif net < -5:
        st.markdown(f"<div class='insight-danger'>⚠️ <b>Negative impact.</b> This worsens your overall score by <b>{abs(net):.1f} points</b>. Think twice.</div>",
                    unsafe_allow_html=True)
    else:
        st.markdown("<div class='insight-warn'>🔄 <b>Neutral impact.</b> Minimal effect on your overall score.</div>",
                    unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown("<p style='color:rgba(255,255,255,0.45)'>Simulates how your metrics evolve over 7 days if you maintain current habits. Fatigue accumulates — watch where you're headed.</p>",
                unsafe_allow_html=True)
    ...
    if countdown:
        ...
        <div class='insight-danger'>
          🚨 <b>Burnout Warning:</b> At your current pace, burnout crosses the critical threshold on
          <b>Day {countdown} ({DAYS[countdown-1]})</b>. Intervene now.
        </div>""", unsafe_allow_html=True)
    else:
        peak = proj["burnout"].max()
        st.markdown(f"""
        <div class='insight-good'>
          ✅ <b>Stable week ahead.</b> Burnout peaks at <b>{peak:.0f}%</b> — well below the danger zone!
        </div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("#### Day-by-day breakdown")
    display = proj.copy()
    display.columns = ["Day", "Burnout Risk %", "Productivity %", "Wellbeing %"]
    st.dataframe(
        display.style
            .background_gradient(subset=["Burnout Risk %"], cmap="RdYlGn_r")
            .background_gradient(subset=["Productivity %", "Wellbeing %"], cmap="RdYlGn")
            .format("{:.1f}", subset=["Burnout Risk %", "Productivity %", "Wellbeing %"]),
        use_container_width=True,
        hide_index=True,
    )
...
with tab4:
    ...
    insights = []
    if sleep < 6:
        insights.append(("danger", "🛏️ <b>Critical Sleep Deficit.</b> Under 6 hours is the #1 predictor of burnout. Aim for 7–8 hours."))
    elif sleep < 7:
        insights.append(("warn", "🛏️ <b>Sleep slightly low.</b> Adding 1 hour per night can reduce burnout risk by up to 15 points."))
    else:
        insights.append(("good", "🛏️ <b>Solid sleep routine.</b> Your sleep is in the healthy range — your biggest protective factor."))

    if screen > 8:
        insights.append(("danger", "📱 <b>Screen time overload.</b> 8+ hours correlates strongly with elevated stress. Cut by 2 hours."))
    elif screen > 6:
        insights.append(("warn", "📱 <b>Screen time elevated.</b> Reduce by 1–2 hours, especially before bed."))

    if stress_enc == 3:
        insights.append(("danger", "🧠 <b>High stress detected.</b> Your biggest risk factor. Even 10 min of daily meditation helps significantly."))
    elif stress_enc == 2:
        insights.append(("warn", "🧠 <b>Moderate stress.</b> Journaling or breathing exercises can shift this to Low within 2 weeks."))

    if activity_enc == 0:
        insights.append(("warn", "🏃 <b>No physical activity.</b> A 20-minute daily walk increases wellbeing by ~10–15 points."))
    else:
        insights.append(("good", "🏃 <b>Active lifestyle.</b> Physical activity is one of your strongest wellbeing boosters. Stay consistent."))

    if caffeine > 3:
        insights.append(("warn", f"☕ <b>High caffeine ({caffeine} cups).</b> Excess caffeine raises cortisol. Stay under 2–3 cups."))
    ...
    if rec:
        ...
        <div class='insight-blue'>
          <div style='font-size:1.3rem;font-weight:800;color:#93c5fd;margin-bottom:8px'>{label}</div>
          <div style='color:rgba(255,255,255,0.7);line-height:1.7'>
            This single change improves your overall life score by
            <b style='color:#34d399'>+{gain:.1f} points</b>.<br>
